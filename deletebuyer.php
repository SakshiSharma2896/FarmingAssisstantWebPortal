...
<div class="dropdown">
  ...
  <a href="homepage.html" style="font-size:28px;color:white;text-decoration:none;font-family:georgian;margin-left:100px;margin-top:-5px">LOGOUT</a>  
	...
	<?php
	//$fetchQuery = mysqli_query("select * from buyerregistration ") or die("could not fetch" . mysql_error());
	...
	?>	
			...
			<label><b><font size="15">DELETE BUYER</font></b></label><br><br>
			<?php
			if(isset($_POST['delete'])){
				$key = $_POST['keytodelete'];
				//check if records to delete exist or not
				$check = mysqli_query($conn, "select * from buyerregistration where ID='$key'") or die("not found" .mysql_error());
				if(mysqli_num_rows($check)>0){
					//means record found delete it
					$querydelete = mysqli_query($conn,"DELETE from buyerregistration where ID= '$key' ") or die("not deleted" .mysql_error());
				?>
